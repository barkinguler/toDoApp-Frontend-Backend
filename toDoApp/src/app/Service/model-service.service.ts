...
export class ModelServiceService {
  ...
  onAddDate(event :ImodelDate){
    event.auth={id:parseInt(localStorage.getItem("id")),username:localStorage.getItem("username"),password:localStorage.getItem("password")};
    this.http.post<ImodelDate>(ipaddress.value+"/post", event).subscribe(
      responseData => {
  ...
  getDates(){
    ...
    this.http.get(ipaddress.value+"/get",{params: params}).pipe(
      map(responseData => {
...
const params = new HttpParams().append('id',value.id.toString() );
    ...
    this.http.get(ipaddress.value+"/get1",{params: params}).pipe(
      map(responseData => {
        const postArray = [];
        for (const key in responseData) {
          if (responseData.hasOwnProperty(key)) {
 ...
 getDateValue(){
  ...
  addItem(event: ImodelWork) {
    ...
    this.http.post(ipaddress.value+"/post1", event).subscribe(
      responseData => {
  ...
  updateItem(event: ImodelWork) {
    console.log(event);
    this.http.put(ipaddress.value+"/update", event).subscribe(
      responseData => {
  ...
  deleteDate(id:number){
    ...
    this.http.delete(ipaddress.value+"/delete",{params: params}).subscribe(
      responseData => {
  ...
  deleteWork(value:ImodelWork,value1:ImodelDate){
    ...
    this.http.delete(ipaddress.value+"/deletework",{params: params}).subscribe(
      responseData => {
  ...
  updateName(event: any){
    ...
    this.http.put(ipaddress.value+"/updateName", event).subscribe(
      responseData => {
        console.log(responseData);
      }
  ...
  updateDateName(event:ImodelWork,valuePreDate:ImodelDate){
    ...
    this.http.put(ipaddress.value+"/updateDateName", event).subscribe(
      responseData => {
  ...
  updatePassword(value:AuthRequestData){
    return this.http
      .post(
        ipaddress.value+'/updatePassword',
        value
      );
